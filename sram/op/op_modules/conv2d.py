...
class SRAMConv2D():
    ...
    def matrix_multiply_with_error(self, input_col, weight_col, parallelism=32, error_range=2):
        ...
        for _ in range(errors_per_output_element):
            random_error = np.round(np.random.uniform(-error_range, error_range, size=output.shape)).astype(np.int8)
            total_errors += random_error
...
if __name__ == "__main__":
    ...
    # noise = True
    noise = True
    input_signal = np.random.randint(0, 2 ** (N-1), size=(32, 256, 64, 64), dtype=np.int8)
    weights = np.random.randint(0, 2 ** (J-1), size=(512, 256, 3, 3), dtype=np.int8)
    ...
    output_tensor = F.conv2d(torch.tensor(input_signal, dtype=torch.int64), torch.tensor(weights, dtype=torch.int64), groups=groups, stride=2, padding=1)
