...
import torchsummary
import math
from AnalogSram.sram_op import sram_einsum
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x,):
        ...
        if hasattr(self, 'parallelism') and hasattr(self, 'error'): # use einsum with SRAM error 
            score = F.softmax(sram_einsum("bhif, bhjf->bhij", q, k, 
                                            parallelism=self.parallelism,
                        error=self.error,)/self.sqrt_d, dim=-1)
            attn = sram_einsum("bhij, bhjf->bihf", score, v, 
                                parallelism=self.parallelism,
                        error=self.error,)
        else:
